...
def create_mask(
    ...
    try:
        ...
        raise KeyError(
            "No operator with name '{}' available! "
            "Available operators: {}"
            "".format(operator_name, ", ".join(OPERATOR_MAP.keys()))
        ) from err
...
TRANSFORMATIONS = {
    ...
    "getattr": lambda d, kws: getattr(
        d, *(kws if not isinstance(kws, str) else [kws])
    ),
    # Item access
    ...
    # Calculate the sum
    "sum": lambda d, kws: d.sum(
        **(kws if isinstance(kws, dict) else dict(dim=kws))
    ),
    # Calculate the mean
    "mean": lambda d, kws: d.mean(
        **(kws if isinstance(kws, dict) else dict(dim=kws))
    ),
    # Calculate the median
    "median": lambda d, kws: d.median(
        **(kws if isinstance(kws, dict) else dict(dim=kws))
    ),
    # Calculate the standard deviation
    "std": lambda d, kws: d.std(
        **(kws if isinstance(kws, dict) else dict(dim=kws))
    ),
    # Calculate the minimum
    "min": lambda d, kws: d.min(
        **(kws if isinstance(kws, dict) else dict(dim=kws))
    ),
    # Calculate the maximum
    "max": lambda d, kws: d.max(
        **(kws if isinstance(kws, dict) else dict(dim=kws))
    ),
    ...
    "create_mask": lambda d, kws: create_mask(
        d,
        **(
            kws
            if isinstance(kws, dict)
            else dict(operator_name=kws[0], rhs_value=kws[1])
        ),
    ),
    # Filter values that do not match a condition, i.e.: make them NaN
    "where": lambda d, kws: where(
        d,
        **(
            kws
            if isinstance(kws, dict)
    ...
    "sqrt3": lambda d, _: np.power(d, 1.0 / 0.3),
    # Normalizations
...
def transform(
    ...
    *operations: Union[dict, str],
    ...
    log.remark(
        "Applying %d transformation%s on %s ...",
        len(operations),
    ...
    for i, op_spec in enumerate(operations):
        if isinstance(op_spec, str):
            ...
            raise TypeError(
                "Entries of `operations` argument need to be dict"
                "-like, but entry {} was {}!"
            ...
            if len(names) != 1:
                raise ValueError(
                    "Could not extract an operation name from "
        ...
        try:
            ...
            raise ValueError(
                "No transform operation with name '{}' is "
                "available! Choose from: {}"
                "".format(op_name, ", ".join(TRANSFORMATIONS.keys()))
            ) from err
        ...
        if isinstance(op_spec, dict) and "rhs_from" in op_spec:
            try:
                ...
                raise ValueError(
                    "The requested data path '{}' for the "
                    "binary operation '{}' was not provided as "
                    "part of the auxilary data. Available "
                    "aux_data: {}"
                    "".format(op_spec["rhs_from"], op_name, aux_data)
                ) from err
    ...
    log.remark(
        "Transformation%s applied.", "s" if len(operations) != 1 else ""
    )
    return data
