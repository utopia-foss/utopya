    ...
    **errorbar_kwargs,
    ...
    if not lines_from:
        ...
        if lines_from not in data.coords:
            log.warning(
                "No labelled '%s' dimension available, only %s. Did "
                "you perhaps not perform a sweep over this dimension?",
                lines_from,
                ", ".join(data.coords.keys()),
            )
        ...
        if cmap is not None:
            ...
            colors = [
                cmap(i / max(num_coords - 1, 1)) for i in range(num_coords)
            ]
        else:
...
def asymptotic_average(
    ...
    **errorbar_kwargs,
    ...
    if data.ndim != 2 or average_dim not in data.dims:
        raise ValueError(
            "Need a 2-dimensional xr.DataArray where one "
    ...
    log.info(
        "Selected '%s' data in asymptotic value interval [%s, %s] "
    ...
    log.info(
        "Plotting %d point(s) over remaining dimension '%s' ...",
        len(mean.coords[iter_dim_name]),
        iter_dim_name,
    )
