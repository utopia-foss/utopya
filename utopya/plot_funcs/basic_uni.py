    ...
    **plot_kwargs,
    ...
    if isinstance(path_to_data, str):
        ...
        if transform_data:
            data_x = transform(
                data_x,
                *transform_data.get("x", {}),
                aux_data=uni["data"][model_name],
                log_level=transformations_log_level,
            )
            data_y = transform(
                data_y,
                *transform_data.get("y", {}),
                aux_data=uni["data"][model_name],
                log_level=transformations_log_level,
            )
        ...
        if data_x.shape != data_y.shape:
            raise ValueError(
                "Mismatch of datashapes! Were {} and {}, but have"
    ...
    if data_x.ndim != 1:
        raise ValueError(
            "Lineplot requires 1D data, but got {}D data of "
    ...
    **common_plot_kwargs,
    ...
    for name, specs in to_plot.items():
        if not isinstance(specs, dict):
            raise TypeError(
                "Parameters for property '{}' were not a dict "
                "but {} with value: '{}'!"
...
def errorbar_single(
    ...
    **errorbar_kwargs,
    ...
    if transform_data:
        data = transform(
            data, *transform_data, log_level=transformations_log_level
        )

    # Get the standard deviation data, if given, and also apply transformations
    std = None
    if plot_std:
        std = uni["data"][model_name][
            plot_std if isinstance(plot_std, str) else path_to_data
        ]

        if transform_std:
            std = transform(
                std, *transform_std, log_level=transformations_log_level
            )
...
@is_plot_func(
    ...
    helper_defaults=dict(set_labels=dict(x="$t_0$", y="$t_1$")),
)
def distance_map(
    dm: DataManager,
    *,
    ...
    **plot_kwargs,
    ...
    if data.ndim != 1:
        raise ValueError(
            "Distance map requires 1D data, but got {}D data of "
            "shape {}:\n{}\n"
            "Apply dimensionality reducing transformations "
            "using the `transform_data` argument to arrive "
            "at plottable data."
