...
def _errorbar(
    ...
    **errorbar_kwargs,
    ...
    if data.ndim != 1:
        raise ValueError(
            "Requiring 1D data to plot a single errorbar "
        ...
        raise ValueError(
            "Requiring 1D standard deviation data to plot the "
    ...
    ebar = hlpr.ax.errorbar(
        data.coords[data.dims[0]], data, yerr=yerr, **errorbar_kwargs
    )
    ...
    if fill_between and std is not None:
        ...
        if "alpha" not in fb_kwargs:
            fb_kwargs["alpha"] = line_alpha * 0.2
