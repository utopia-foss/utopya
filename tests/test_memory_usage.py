...
def gather_memory_info(
    model: Model, *, mv_kwargs: dict = None, update_uni_cfg: dict = None
) -> Dict[str, np.ndarray]:
    ...
    mv.wm.start_working(
        **mv.meta_cfg["run_kwargs"], post_poll_func=check_memory
    )
...
def test_hdf5_write(cfg):
    ...
    for i, case_cfg in enumerate(cfg["test_cases"]):
        ...
        # Gather memory
        mem_data = gather_memory_info(
            model, mv_kwargs=mv_kwargs, update_uni_cfg=case_cfg.get("uni_cfg")
        )
