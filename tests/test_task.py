...
def test_workertask_init():
    ...
    with pytest.warns(UserWarning, match="`worker_kwargs` given but also"):
        WorkerTask(
            name=0, setup_kwargs=dict(foo="bar"), worker_kwargs=dict(foo="bar")
        )
...
def test_workertask_invalid_args():
    ...
    t = WorkerTask(
        name=1,
        worker_kwargs=dict(args=("python", "--version"), save_streams=True),
    )
    ...
    t = WorkerTask(
        name=2, worker_kwargs=dict(args=("/i/do/not/exist", "--arg1"))
    )
    with pytest.raises(
...
def test_enqueue_lines():
    ...
    enqueue_lines(queue=q, stream=io.StringIO("hello"))
    ...
    enqueue_lines(queue=q, stream=io.StringIO("1"))
...
def test_MPProcessTask():
    ...
    t = MPProcessTask(
        name=0, worker_kwargs=dict(args=(pmp_test_target,), read_stdout=True)
    )
